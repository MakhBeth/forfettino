...
export function FatturePage({ setShowModal, setEditingFattura }: FattureProps) {
  ...
  const [ordinamentoFatture, setOrdinamentoFatture] = useState<{
    campo: string;
    direzione: string;
  }>({ campo: "dataIncasso", direzione: "desc" });
  ...
  const fattureFiltrate =
    filtroAnnoFatture === "tutte"
      ? fatture
      : fatture.filter((f) => {
          const dataRiferimento = f.dataIncasso || f.data;
          return (
            new Date(dataRiferimento).getFullYear() ===
            parseInt(filtroAnnoFatture)
          );
        });
  ...
  const handleSort = (campo: string) => {
    setOrdinamentoFatture((prev) => ({
      campo,
      direzione:
        prev.campo === campo && prev.direzione === "asc" ? "desc" : "asc",
    }));
  };
  ...
  const fatturatoPerCliente = clienti
    .map((cliente) => {
      const fattureCliente = fattureAnnoCorrente.filter(
        (f) => f.clienteId === cliente.id,
      );
      return {
        ...cliente,
        totale: fattureCliente.reduce((sum, f) => sum + f.importo, 0),
        count: fattureCliente.length,
      };
    })
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => setShowModal("nuova-fattura")}
            >
              <FilePlus size={18} aria-hidden="true" /> Nuova Fattura
            </button>
            <button
              className="btn btn-primary"
              onClick={() => setShowModal("upload-fattura")}
            >
              ...
              <thead>
                <tr>
                  <th
                    scope="col"
                    aria-sort={
                      ordinamentoFatture.campo === "numero"
                        ? ordinamentoFatture.direzione === "asc"
                          ? "ascending"
                          : "descending"
                        : undefined
                    }
                  ...
                  <th
                    scope="col"
                    aria-sort={
                      ordinamentoFatture.campo === "dataIncasso"
                        ? ordinamentoFatture.direzione === "asc"
                          ? "ascending"
                          : "descending"
                        : undefined
                  ...
                  >
                    <button
                      type="button"
                      onClick={() => handleSort("dataIncasso")}
                    >
                      Data Incasso{" "}
                      {ordinamentoFatture.campo === "dataIncasso" &&
                        (ordinamentoFatture.direzione === "asc" ? "↑" : "↓")}
                  ...
                  <th
                    scope="col"
                    aria-sort={
                      ordinamentoFatture.campo === "clienteNome"
                        ? ordinamentoFatture.direzione === "asc"
                          ? "ascending"
                          : "descending"
                        : undefined
                  ...
                  <th
                    scope="col"
                    aria-sort={
                      ordinamentoFatture.campo === "importo"
                        ? ordinamentoFatture.direzione === "asc"
                          ? "ascending"
                          : "descending"
                        : undefined
                ...
                {fattureOrdinate.map((f) => {
                  ...
                  return (
                    <tr key={f.id}>
                      ...
                      <td>
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              setEditingFattura({ ...f });
                              setShowModal("edit-data-incasso");
                            }}
                      ...
                      <td>
                        {f.clienteNome ||
                          clienti.find((c) => c.id === f.clienteId)?.nome ||
                          "-"}
                      </td>
