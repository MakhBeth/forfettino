...
import { Calculator, TrendingUp, Percent, PiggyBank, Wallet } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { LIMITE_FATTURATO, INPS_GESTIONE_SEPARATA, ALIQUOTA_RIDOTTA, ALIQUOTA_STANDARD, COEFFICIENTI_ATECO } from '../../lib/constants/fiscali';
...
export function Simulatore() {
  ...
  const fatturatoNum = parseFloat(fatturato.replace(/[^\d.,]/g, '').replace(',', '.')) || 0;
  ...
  const calculations = useMemo(() => {
    ...
    const percentualeNetto = fatturatoNum > 0 ? (nettoStimato / fatturatoNum) * 100 : 0;
  ...
  return (
    <>
      <div className="page-header">
        ...
        <p className="page-subtitle">Stima il tuo reddito netto in base al fatturato</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        {isOverLimit && (
          <div style={{
            ...
            <span>Attenzione: superi il limite di &euro;{formatCurrency(LIMITE_FATTURATO)} per il regime forfettario!</span>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px 32px' }}>
            <div>
              ...
              {config.codiciAteco.length > 0 && (
                <span style={{ color: 'var(--text-muted)' }}> (ATECO: {config.codiciAteco.join(', ')})</span>
              )}
            </div>
            <div>
              <span style={{ color: 'var(--text-muted)' }}>Aliquota IRPEF: </span>
              <strong>{(aliquotaIrpef * 100).toFixed(0)}%</strong>
              {config.aliquotaOverride !== null && <span style={{ color: 'var(--text-muted)' }}> (custom)</span>}
              {config.aliquotaOverride === null && anniAttivita < 5 && <span style={{ color: 'var(--accent-green)' }}> (agevolata)</span>}
            </div>
            <div>
      ...
      <div className="grid-4">
        <div className="card">
          <h2 className="card-title">
            <Percent size={16} style={{ marginRight: 6, verticalAlign: 'middle' }} aria-hidden="true" />
            Imponibile
          </h2>
          <div className="stat-value" style={{ color: 'var(--text-primary)' }}>
            &euro;{formatCurrency(calculations.imponibile)}
          </div>
          <div className="stat-label">
            {coefficienteMedio}% del fatturato
          </div>
        </div>

        <div className="card">
          <h2 className="card-title">
            <Calculator size={16} style={{ marginRight: 6, verticalAlign: 'middle' }} aria-hidden="true" />
            IRPEF
          </h2>
          <div className="stat-value" style={{ color: 'var(--accent-orange)' }}>
            &euro;{formatCurrency(calculations.irpef)}
          </div>
          <div className="stat-label">
        ...
        <div className="card">
          <h2 className="card-title">
            <PiggyBank size={16} style={{ marginRight: 6, verticalAlign: 'middle' }} aria-hidden="true" />
            INPS
          </h2>
          <div className="stat-value" style={{ color: 'var(--accent-orange)' }}>
            &euro;{formatCurrency(calculations.inps)}
          </div>
          <div className="stat-label">
        ...
        <div className="card">
          <h2 className="card-title">
            <Wallet size={16} style={{ marginRight: 6, verticalAlign: 'middle' }} aria-hidden="true" />
            Totale Tasse
          </h2>
          <div className="stat-value" style={{ color: 'var(--accent-red)' }}>
            &euro;{formatCurrency(calculations.totaleTasse)}
          </div>
          <div className="stat-label">
            IRPEF + INPS
          </div>
      ...
      <div className="grid-3">
        <div className="card" style={{
          background: 'linear-gradient(135deg, var(--bg-card) 0%, rgba(4,120,87,0.1) 100%)'
        }}>
          <h2 className="card-title">Reddito Netto Stimato</h2>
          <div className="stat-value" style={{ fontSize: '2.4rem', color: 'var(--accent-green)' }}>
            &euro;{formatCurrency(calculations.nettoStimato)}
          </div>
          <div className="stat-label">
            Fatturato meno tasse
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="stat-value" style={{ color: 'var(--text-primary)' }}>
            &euro;{formatCurrency(calculations.nettoStimato / 12)}
          </div>
          <div className="stat-label">
            Diviso 12 mesi
          </div>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ textAlign: 'right', fontFamily: 'Space Mono, monospace' }}>
                    &euro;{formatCurrency(fatturatoNum)}
                  </td>
                ...
                <tr>
                  ...
                  <td style={{ textAlign: 'right', color: 'var(--text-muted)' }}>
                    {(calculations.imponibile / fatturatoNum * 100).toFixed(1)}%
                  </td>
                </tr>
                <tr>
                  ...
                  <td style={{ textAlign: 'right', fontFamily: 'Space Mono, monospace', color: 'var(--accent-orange)' }}>
                    -&euro;{formatCurrency(calculations.irpef)}
                  </td>
                  <td style={{ textAlign: 'right', color: 'var(--text-muted)' }}>
      ...
      {fatturatoNum > 0 && (
        <div className="card">
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <span style={{ color: 'var(--text-muted)' }}> / &euro;{formatCurrency(LIMITE_FATTURATO)}</span>
          </div>
